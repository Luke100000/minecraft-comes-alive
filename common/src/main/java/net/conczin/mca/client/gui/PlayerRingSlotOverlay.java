...
public final class PlayerRingSlotOverlay {
   ...
   public static void render(GuiGraphics context, int mouseX, int mouseY, int slotX, int slotY) {
      ...
      if (minecraft.player != null) {
         ...
         if (ring.isEmpty()) {
            context.drawString(minecraft.font, "R", slotX + 6, slotY + 5, hovering ? -1 : -1329018680, false);
         } else {
            context.renderItem(ring, slotX + 1, slotY + 1);
            context.renderItemDecorations(minecraft.font, ring, slotX + 1, slotY + 1);
         }
      }
   }

   public static void renderTooltip(GuiGraphics context, int mouseX, int mouseY, int slotX, int slotY) {
