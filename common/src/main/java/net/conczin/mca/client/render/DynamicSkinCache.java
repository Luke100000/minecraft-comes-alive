...
public final class DynamicSkinCache {
    ...
    private static boolean isMissingImmersiveLibraryAssets(VillagerLike<?> villager) {
        if (isMissingImmersiveLibraryAsset(villager.getClothes())) {
            return true;
        }
        if (isMissingImmersiveLibraryAsset(villager.getHair())) {
            return true;
        }
    ...
    private record SkinKey(
        ...
        static SkinKey from(Entity entity, VillagerLike<?> villager) {
            return new SkinKey(
                    ...
                    villager.getGenetics().getGene(Genetics.EYE_COLOR),
