...
final class BlueprintTerrainRenderer {
    ...
    private void renderTile(GuiGraphicsExtractor context, TerrainTile tile) {
        if (tile.terrainTextureLocation == null) return;

        context.blit(RenderPipelines.GUI_TEXTURED, tile.terrainTextureLocation, tile.minX, tile.minZ,
                0.0F, 0.0F, TILE_BLOCK_SIZE, TILE_BLOCK_SIZE,
    ...
    private void updateTexture(TerrainTile tile) {
        if (tile.terrainTexture == null) {
            ...
            Identifier terrainTextureLocation = Identifier.fromNamespaceAndPath(
                    "mca", "dynamic/blueprint_terrain/" + tile.minX + "_" + tile.minZ);
            DynamicTexture terrainTexture = new DynamicTexture(terrainTextureLocation::toString, terrainImage);
            tile.terrainTexture = terrainTexture;
            tile.terrainTextureLocation = terrainTextureLocation;
            Minecraft.getInstance().getTextureManager().register(terrainTextureLocation, terrainTexture);
            return;
        }
        ...
        for (int pixelX = 0; pixelX < TILE_BLOCK_SIZE; pixelX++) {
            for (int pixelZ = 0; pixelZ < TILE_BLOCK_SIZE; pixelZ++) {
                ...
                if (!tile.isCellValid(cellX, cellZ)) {
                    terrainImage.setPixel(pixelX, pixelZ, 0);
                    continue;
                }
                ...
                terrainImage.setPixel(pixelX, pixelZ, color);
            }
        }
    }
    ...
    private static final class TerrainTile {
        ...
        private Identifier terrainTextureLocation;
