...
public class Village implements Iterable<Building> {
    ...
    public Village(CompoundTag tag, ServerLevel world) {
        ...
        name = tag.getString("name").orElse(name);
        chatAIPrompt = tag.getString("chatAIPrompt").orElse("");
        taxes = tag.getFloat("taxesFloat").orElse(0.0F);
        beds = tag.getInt("beds").orElse(0);
        reputation = NbtHelper.toMap(tag.getCompound("reputation").orElseGet(CompoundTag::new), UUID::fromString, value ->
                NbtHelper.toMap((CompoundTag) value, UUID::fromString, inner -> ((IntTag) inner).intValue()));
        residentNames = NbtHelper.toMap(tag.getCompound("residentNames").orElseGet(CompoundTag::new), UUID::fromString,
                value -> value.asString().orElse(""));
        residentHomes = NbtHelper.toMap(tag.getCompound("residentHomes").orElseGet(CompoundTag::new), UUID::fromString,
                value -> ((LongTag) value).longValue());
        if (tag.contains("populationThresholdFloat")) populationThreshold = tag.getFloat("populationThresholdFloat").orElse(populationThreshold);
        if (tag.contains("marriageThresholdFloat")) marriageThreshold = tag.getFloat("marriageThresholdFloat").orElse(marriageThreshold);
        autoScan = tag.getBoolean("autoScan").orElse(true);
        nextMourningTime = tag.getLong("nextMourningTime").orElse(0L);
        this.world = world;
    ...
    private void tickMourning(ServerLevel world, long time) {
        ...
        if (nextMourningTime == 0L) {
            nextMourningTime = calculateNextMourningTime(time, world.getRandom());
            markDirty();
            return;
        }

        if (time < nextMourningTime || !isAmbientMourningTime(world.getOverworldClockTime())) {
            return;
        }
    ...
    private void releaseMourningBurst(ServerLevel world, long time) {
        ...
        Util.shuffle(graves, world.getRandom());
        Util.shuffle(candidates, world.getRandom());
        ...
        int count = Math.min(calculateMourningBurstSize(world.getRandom()), candidates.size());
