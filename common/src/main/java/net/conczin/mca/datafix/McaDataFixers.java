...
public final class McaDataFixers {
    ...
    public static CompoundTag update(CompoundTag input) {
        ...
        CompoundTag nested = updated.getCompound(LEGACY_MCA_DATA_KEY).orElse(null);
        if (nested == null) {
            return updated;
        }
    ...
    public static void stampCurrentVersion(CompoundTag output) {
        ...
        stampPayload(output);
        CompoundTag nested = output.getCompound(LEGACY_MCA_DATA_KEY).orElse(null);
        if (nested != null) {
            stampPayload(nested);
    ...
    private static int getVersion(CompoundTag input) {
        return input.getInt(DATA_VERSION_KEY).orElse(0);
    }
