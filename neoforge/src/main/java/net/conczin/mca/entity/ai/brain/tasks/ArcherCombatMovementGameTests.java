...
public final class ArcherCombatMovementGameTests {
    ...
    public static void emergencyFleeReplacesPreCombatWalkTarget(GameTestHelper helper) {
        cleanupTestEntities();
        ...
        WalkTarget preCombatTarget = new WalkTarget(Vec3.atBottomCenterOf(start.east(10)), 0.5F, 0);
        ...
        WalkTarget combatTarget = archer.getBrain().getMemory(MemoryModuleType.WALK_TARGET).orElse(null);
        helper.assertTrue(combatTarget != null, "EMERGENCY_FLEE lost WALK_TARGET while taking combat ownership");
        helper.assertTrue(combatTarget != preCombatTarget, "EMERGENCY_FLEE preserved the pre-combat WALK_TARGET");
        helper.assertTrue(
                combatTarget.getTarget().currentPosition().distanceToSqr(target.position()) > archer.distanceToSqr(target),
                "EMERGENCY_FLEE replacement WALK_TARGET did not open distance from the threat"
        );
        helper.succeed();
    }
    ...
    public static void holdStopsPreCombatNavigation(GameTestHelper helper) {
        cleanupTestEntities();
        BlockPos start = helper.absolutePos(new BlockPos(8, 2, 8));
        prepareFlatArea(helper, start, 12);
        VillagerEntityMCA archer = spawnArcher(helper, start);
        ...
        archer.getBrain().setMemory(
                ...
                new WalkTarget(Vec3.atBottomCenterOf(start.north(8)), 0.5F, 0)
        );
    ...
    public static void emergencyFleePublishesEscapeFromNearbyThreatGroup(GameTestHelper helper) {
        ...
        archer.getBrain().setMemory(
                ...
                new net.minecraft.world.entity.ai.memory.NearestVisibleLivingEntities(helper.getLevel(), archer, List.of(eastThreat, westThreat))
        );
        ...
        helper.onEachTick(() -> {
            if (!hurt[0]) {
                if (RangedCombatState.current(archer).isPresent()
                    ...
                    helper.assertTrue(
                            archer.hurtServer(helper.getLevel(), helper.getLevel().damageSources().mobAttack(target), 1.0F),
    ...
    public static void kiteBandDoesNotPingPong(GameTestHelper helper) {
        ...
        target.snapTo(archer.getX() + 5.5D, archer.getY(), archer.getZ());
        ...
        for (int i = 1; i <= 12; i++) {
            target.snapTo(archer.getX() + 7.0D, archer.getY(), archer.getZ());
        ...
        for (int i = 13; i <= 24; i++) {
            target.snapTo(archer.getX() + 8.9D, archer.getY(), archer.getZ());
            task.tick(helper.getLevel(), archer, gameTime + i);
            helper.assertTrue(
                    RangedCombatState.current(archer).orElse(null) == RangedCombatState.KITE,
        ...
        target.snapTo(archer.getX() + 9.0D, archer.getY(), archer.getZ());
    ...
    public static void attackRangeBoundaryDoesNotPingPongApproachAndHold(GameTestHelper helper) {
        ...
        target.snapTo(archer.getX() + 15.05D, archer.getY(), archer.getZ());
        ...
        target.snapTo(archer.getX() + 14.95D, archer.getY(), archer.getZ());
    ...
    public static void emergencyFleeCancelsBowDraw(GameTestHelper helper) {
        ...
        helper.onEachTick(() -> {
            if (!drawStarted[0] && archer.isUsingItem()) {
                drawStarted[0] = true;
                target.snapTo(archer.getX() + 2.0D, archer.getY(), archer.getZ());
                return;
            }
    ...
    private static Zombie spawnTarget(GameTestHelper helper, BlockPos pos) {
        ...
        target.snapTo(pos.getX() + 0.5D, pos.getY(), pos.getZ() + 0.5D);
        target.setNoAi(true);
